                ...
                >
                  ...
                  {activeReplyCommentId === comment.id ? (
                    <div className="mt-3 space-y-2 rounded-md border border-slate-200 bg-white p-3">
                      <label className="block text-xs font-medium text-slate-600" htmlFor={`reply-${comment.id}`}>
                        Reply
                      </label>
                      <textarea
                        id={`reply-${comment.id}`}
                      ...
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <Button
                          ...
                          disabled={!email || isReplySubmitting || !(replyInputs[comment.id] ?? "").trim()}
                        >
                  ...
                  {comment.replies.length > 0 ? (
                    <div className="mt-3 space-y-2 border-l-2 border-slate-200 pl-3">
                      {comment.replies.map((reply) => (
                        <article key={reply.id} className="rounded-md border border-slate-200 bg-white px-3 py-2">
                          <header className="flex flex-wrap items-center justify-between gap-2">
                            <div className="flex items-center gap-2">
                              <p className="text-sm font-medium text-slate-800">{reply.commentatorName}</p>
                          ...
                          <p className="mt-1 text-sm whitespace-pre-wrap text-slate-700">{reply.comment}</p>
                          <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
                            <div className="text-xs text-slate-500">
